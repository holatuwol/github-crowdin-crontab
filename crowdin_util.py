...
x_csrf_token = "".join(
    [random.choice("0123456789abcdefghijklmnopqrstuvwxyz") for x in range(10)]
)
...
def crowdin_authenticate(path):
    ...
    while r.text.find("resend_device_verification_code") != -1:
        ...
        r = session.post(
            "https://accounts.crowdin.com/device-verify/code",
            data=login_data,
    ...
    if r.text.find("/remember-me/decline") != -1:
        ...
        login_data = {"_token": token_input.attrs["value"]}

        r = session.post(
            "https://accounts.crowdin.com/remember-me/decline",
            data=login_data,
            headers={"x-csrf-token": x_csrf_token},
        )
...
def get_crowdin_file(repository, local_file):
    if local_file.find(repository.git_folder) == 0:
        return (
            repository.dest_folder + "/" + local_file[len(repository.git_folder) + 1 :]
        )
...
def get_crowdin_file_info(repository, target_language):
    ...
    for item in response_data:
        item_path = (
            item["data"]["path"]
            if repository.dest_folder[0] == "/"
            else item["data"]["path"][1:]
        )
    ...
    logging.info(
        "Found %d/%d files matching destination folder %s"
        % (len(file_info), len(response_data), repository.dest_folder)
    )

    # Fetch the list of translation statuses

    pagination_data["offset"] = 0
    api_path = "/projects/%s/languages/%s/progress" % (
    ...
    while True:
        ...
        for item in response_data:
            ...
            if item_path in file_info:
                ...
                file_info[item_path]["translated"] = item["data"]["phrases"][
                    "translated"
                ]
...
def get_directory(repository, subfolder, create_if_missing=True):
    ...
    while (
        parent_path not in directory_paths and parent_path != "/" and parent_path != ""
    ):
...
def get_repositories():
    ...
    repositories = [
        get_repository_helper(projects, **x) for x in repositories_df.to_dict("records")
    ]
...
def get_repository_helper(
    ...
    project_api_keys = [
        project["key"] if "key" in project else None
...
def get_translation_path(file, source_language, target_language):
    ...
    if file[0:3] == "%s/" % source_language_path:
        ...
        return file.replace(
            "/%s/" % source_language_path, "/%s/" % target_language_path
        )
