...
def requires_new_english_translation(utf8_string):
    ...
    for i, ch in enumerate(utf8_string):
        if (
            ord(ch) > 127
            and ch not in ignore_characters
            and ch not in japanese_punctuation
        ):
            tag = get_previous_tag(utf8_string, i)
            if tag != "code" and tag != "pre":
                return True
...
def get_zendesk_article(domain, article_id, target_language):
    api_path = "/help_center/articles/%s/translations/%s.json" % (
        article_id,
        target_language,
    )
...
def get_new_articles(domain):
    ...
    new_start_time = (
        ...
        else max([article["updated_at"] for article in all_articles.values()])
    )

    article_parameters = {"start_time": new_start_time}

    new_article_list = zendesk_get_request(
        domain, "/help_center/incremental/articles.json", "articles", article_parameters
    )

    new_articles = {str(article["id"]): article for article in new_article_list}
...
def fix_author_source_locale(
    repository, domain, bad_language, good_language, author_id
):
    bad_articles = None

    if os.path.exists("check_only_articles.txt"):
        with open("check_only_articles.txt", "r") as f:
            fix_articles = [line.strip() for line in f.readlines()]
    else:
        with open(
            "%s/zendesk/articles_%s_%s.json" % (initial_dir, bad_language, domain), "r"
        ) as f:
            bad_articles = json.load(f)

        fix_articles = [
            article_id
            for article_id, article in bad_articles.items()
    ...
    for article_id in fix_articles:
        try:
            api_path = "/help_center/%s/articles/%s/source_locale.json" % (
                bad_language,
                article_id,
            )

            json_params = {"article_locale": good_language}
            print(json_params)
    ...
    if bad_articles is not None:
        with open(
            "%s/zendesk/articles_%s_%s.json" % (initial_dir, bad_language, domain), "w"
        ) as f:
            json.dump(bad_articles, f)


def get_zendesk_articles(
    repository, domain, source_language, target_language, fetch_update
):
    ...
    section_paths = {
        section_id: "%s%s"
        % (
            categories[str(section["category_id"])]["name"],
            section["html_url"][section["html_url"].rfind("/") :],
        )
    ...
    try:
        with open(
            "%s/zendesk/articles_%s_%s.json" % (initial_dir, source_language, domain),
            "r",
        ) as f:
    ...
    with open(
        "%s/zendesk/articles_%s_%s.json" % (initial_dir, source_language, domain), "w"
    ) as f:
...
def copy_crowdin_to_zendesk(
    repository, domain, source_language, target_language, authors=None
):
    articles, new_tracked_articles, categories, sections, section_paths = (
        get_zendesk_articles(repository, domain, source_language, target_language, True)
    )

    old_dir = os.getcwd()

    article_paths = check_renamed_articles(
        repository, source_language, target_language, articles, section_paths
    )

    if os.path.exists("check_only_articles.txt"):
        with open("check_only_articles.txt", "r") as f:
            updated_source_files = [
                article_paths[line.strip()] for line in f.readlines()
            ]
    else:
        updated_source_files = [
            article_paths[article_id] for article_id, article in articles.items()
        ]
    ...
    missing_target_language_article_ids = [
        article["id"]
    ...
    logging.info(
        "%d updated files, %d missing %s label"
        % (
            len(updated_target_files),
            len(missing_target_language_article_ids),
            target_language,
        )
    )
    ...
    for article_id, source_file in sorted(article_paths.items()):
        ...
        target_file = get_translation_path(
            source_file, source_language, target_language
        )

        if (
            target_language in article["label_names"]
            and target_file not in updated_target_files
        ):
            continue
        ...
        if authors is None or str(article["author_id"]) in authors:
            if authors is not None:
                logging.info(
                    "%s (updating since author %s is included)"
            ...
            logging.info(
                "%s (skipping since author %s is excluded)"
                % (article_id, article["author_id"])
            )
...
def copy_zendesk_to_crowdin(repository, domain, source_language, target_language):
    articles, article_paths, refresh_articles, refresh_paths = (
        download_zendesk_articles(
            repository, domain, source_language, target_language, False
        )
    )
    ...
    if len(refresh_paths) > 0:
        ...
        crowdin_upload_sources(
            repository, source_language, target_language, refresh_paths.values()
        )
    else:
...
def check_renamed_articles(
    ...
    new_article_paths = {
        str(article["id"]): get_article_path(
            article, source_language_path, section_paths
        )
        for article in articles.values()
    ...
    new_translated_paths = {
        str(article["id"]): get_article_path(
            article, target_language_path, section_paths
        )
        for article in articles.values()
        if is_tracked_article(article, source_language, section_paths)
    }
...
def save_article_metadata(
    domain, repository, target_language, articles, article_paths, categories, sections
):
    target_language_path = target_language
...
def get_categories(domain, target_language):
    ...
    category_list = zendesk_get_request(
        domain, "/help_center/en-us/categories.json", "categories"
    )
    ...
    for category in category_list:
        ...
        if (
            ...
            and "title_" + target_language in categories[category_id]
        ):
            continue

        translations = zendesk_get_request(
            domain,
            "/help_center/categories/%s/translations/%s.json"
            % (category_id, target_language),
            "translation",
        )
...
def get_sections(domain, target_language):
    ...
    section_list = zendesk_get_request(
        domain, "/help_center/en-us/sections.json", "sections"
    )
    ...
    for section in section_list:
        ...
        if (
            ...
            and "title_" + target_language in sections[section_id]
        ):
            continue

        translations = zendesk_get_request(
            domain,
            "/help_center/sections/%s/translations/%s.json"
...
def download_zendesk_articles(
    repository, domain, source_language, target_language, fetch_update=False
):
    user = init_zendesk(domain)
    logging.info("Authenticated as %s" % user["email"])
    assert user["verified"]
    ...
    if os.path.exists(os.path.join(old_dir, "check_only_articles.txt")):
        with open(os.path.join(old_dir, "check_only_articles.txt"), "r") as f:
            refresh_articles = {
                line.strip(): articles[line.strip()] for line in f.readlines()
            }
    else:
        refresh_articles = {
            article_id: article
    ...
    refresh_paths = {
        article_id: article_paths[article_id] for article_id in refresh_articles.keys()
    }
...
def remove_nbsp(content):
    ...
    content = new_content
    new_content = new_content.replace("</span><span>", "").replace(
        "</span> <span>", " "
    )
...
def requires_update(
    ...
    if target_language not in article["label_names"]:
        logging.info(
            "%s (requires update check: missing %s label)"
    ...
    if not os.path.exists(target_path):
        logging.info(
            "%s (requires update check: no translation on file system %s)"
            % (article["id"], target_path)
        )
        return True
    ...
    new_title, old_content, new_content = add_disclaimer_zendesk(
        article, target_file, target_language
    )
    ...
    if fetch_update:
        ...
        if mt_article is None:
            logging.info(
                "%s (requires update check: missing %s translation on zendesk)"
                % (article["id"], target_language)
            )
            return True

        mt_article_text = (
            BeautifulSoup(mt_article["body"], features="html.parser").get_text().strip()
        )

        if mt_article_text.find(disclaimer[target_language]) == -1:
            logging.info(
                "%s (requires update check: missing MT disclaimer)" % article["id"]
            )
            return True

        if mt_article["body"] != new_content:
            logging.info(
                "%s (requires update check: mismatched translated content)"
                % article["id"]
            )
            return True

    logging.info(
        "(requires update check %s: no update required %s)"
        % (article["id"], target_file)
    )

    return False


def update_zendesk_translation(
    repository,
    domain,
    article,
    source_file,
    ...
    if not os.path.exists(target_file):
        print(
            "%s (skipping translation, not available on file system %s)"
            % (article["id"], target_file)
        )
        return False
    ...
    if target_language in article["label_names"] and "mt" not in article["label_names"]:
        print(
            "%s (skipping translation, not supposed to be updated since it was manually translated)"
            % article["id"]
        )
        return False
    ...
    if not requires_update(
        ...
        logging.info(
            "%s (skipping translation, file is up to date %s)"
            % (article["id"], target_file)
        )
        return False
    ...
    if missing_locales is None or target_language in missing_locales:
        ...
        json_params = {"translation": {"title": new_title, "body": new_content}}
...
if __name__ == "__main__":
    try:
        ...
        translate_zendesk_on_crowdin(
            repository, domain, source_language, target_language
        )
        download_zendesk_articles(
            repository, domain, source_language, target_language, True
        )
