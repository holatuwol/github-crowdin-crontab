...
def authorize():
	...
	client_id = onepass.item('OAuth2 %s' % os.getenv('learn_domain'), 'username')['username']
	client_secret = onepass.item('OAuth2 %s' % os.getenv('learn_domain'), 'credential')['credential']
	...
	r = session.post(f'https://{learn_domain}/o/oauth2/token', data=params, headers={'User-Agent': 'translate_learn.py'})
...
def make_headless_list_request(url, accept_language, initial_params):
	...
	status_code, response_json = make_headless_request(url, 'GET', accept_language, params)
...
def download_updated_articles(language):
	...
	if os.path.exists(cache_file):
		...
		params = {
			'flatten': 'true',
			'filter': f'dateModified ge {last_search_time}'
		}
	...
	for article in new_articles:
		...
		with open('%s/%s.json' % (language_folder, article['id']), 'w', encoding='utf-8') as f:
			json.dump(article, f)

		content = content_fields[0]['contentFieldValue']['data']

		with open('%s/%s.html' % (language_folder, article['id']), 'w', encoding='utf-8') as f:
			f.write(content)
...
def copy_learn_to_crowdin(source_language, target_language):
	...
	crowdin_upload_sources(repository, source_language[:2], target_language[:2], outdated_articles[:50])
...
def translate_learn_on_crowdin(source_language, target_language):
	...
	pre_translate(repository, source_language[:2], target_language[:2], outdated_articles, file_info)
...
def copy_crowdin_to_local(source_language, target_language):
	...
	export_file_name = crowdin_download_translations(repository, source_language[:2], target_language[:2], outdated_articles, file_info)

	extract_crowdin_translation(repository, export_file_name, source_language[:2], source_language[:2])
...
def publish_target_content(article_id, source_language, target_language):
	...
	if target_language in disclaimer:
		update_content = '<div class="adm-block adm-note">%s</div>%s' % (disclaimer[target_language], html_content)
	...
	content_field = [field for field in params['contentFields'] if field['name'] == 'content'][0]
	content_field['contentFieldValue']['data'] = update_content

	if target_language in available_languages:
		logging.info('Updating translation to %s on article %s' % (target_language, article_id))

		status_code, response_data = make_headless_request(update_article_url, 'PATCH', target_language, params)
	else:
		logging.info('Adding translation to %s on article %s' % (target_language, article_id))
		logging.info(json.dumps(params))

		status_code, response_data = make_headless_request(update_article_url, 'PUT', target_language, params)
